...
void VNode::PrintPolicyTree(int depth, vector<int>& policyStar, vector<int>& depthOrder) {
    depthOrder.push_back(this->depth());
	if (depth != -1 && this->depth() > depth)
		return;
	...
	if (qnodes.size() == 0) {
		int astar = this->default_move().action;
		policyStar.push_back(astar);
		//cout << this << "-da=" << astar << endl;
	} else {
		QNode* qstar = NULL;
		for (int a = 0; a < qnodes.size(); a++) {
			QNode* qnode = qnodes[a];
        ...
        policyStar.push_back(qstar->edge());
		//cout << this << "-a=" << qstar->edge() << endl;
		...
		for (int i = 0; i < labels.size(); i++) {
			if (depth == -1 || this->depth() + 1 <= depth) {
				//cout << repeat("|   ", this->depth()) << "| o=" << labels[i] << ": ";
				qstar->Child(labels[i])->PrintPolicyTree(depth, policyStar, depthOrder);
			}
		}
	}
}
