...
const double initi_obs_x = -20 + (0.65 - 0.5)*20;
const double initi_obs_y = 4;
...
POMDP_Plan::POMDP_Plan(){
	...
	vector<double> y_A_0 = {initi_obs_y, 4};
	vector<double> theta_A_0 = {0, 0};
	vector<double> x_A_1 = {initi_obs_x, 20, 23, 23};
...
/// TODO (bingyu#5#): need to change the obs_prob for obs 3 and 2

    if (pomdp_state.state_A[0] <= 21.7){
        if (action == Dec){
...
State* POMDP_Plan::CreateStartState(std::string type) const{
	...
	state_A = {initi_obs_x, initi_obs_y , 0, 3, 0, 0};
...
Belief* POMDP_Plan::InitialBelief(const State* start, std::string type) const{
	...
	straight_state->state_A = {initi_obs_x, initi_obs_y , 0, 3, 0, 0};
	...
	turning_state->state_A = {initi_obs_x, initi_obs_y , 0, 3, 1, 0};
	...
	stop_state->state_A = {initi_obs_x, initi_obs_y , 0, 3, 2, 0};
